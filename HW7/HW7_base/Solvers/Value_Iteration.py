...
class ValueIteration(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for each_state in range(self.env.nS):
            ...
            val = 0.0
            for id in range(len(self.env.P[each_state][action])):
                probability, next_state, reward, done = self.env.P[each_state][action][id]
                val += probability*(reward+ self.options.gamma* self.V[next_state])

            self.V[each_state] = val
    ...
    def create_greedy_policy(self):
        ...
        def policy_fn(state):
            ...
            for action in range(self.env.nA):
                val = 0.0
                for id in range(len(self.env.P[state][action])):
                    probability, next_state, reward, done = self.env.P[state][action][id]
                    val += probability*(reward+ self.options.gamma* self.V[next_state])
                vals.append(val)
