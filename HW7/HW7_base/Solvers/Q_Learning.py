...
class QLearning(AbstractSolver):
    ...
    def train_episode(self):
        ...
        while True:
            ...
            q_next_state_max = np.max(self.Q[next_state])
            val_update = reward + self.options.gamma*q_next_state_max - self.Q[state][action]
            self.Q[state][action] += self.options.alpha*(val_update)
    ...
    def epsilon_greedy_action(self, state):
        ...
        #   YOUR IMPLEMENTATION HERE   #
        ################################
        action_probs = np.zeros(self.env.nA)
        if np.random.random() > self.options.epsilon:
            best_action = np.argmax(self.Q[state])
            action_probs[best_action] = 1
        else:
            for action in range(self.env.nA):
                action_probs[action] = 1/(self.env.nA)     
